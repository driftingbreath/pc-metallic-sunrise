...
if DEF(FAITHFUL)
	...
	move DOUBLE_SLAP,     EFFECT_DOUBLE_HIT,         30, FAIRY,     100, 15,   0, PHYSICAL ; it's DOUBLEslap. also more physical Fairy please
endc
...
if DEF(FAITHFUL)
	...
	move X_SCISSOR,       EFFECT_NORMAL_HIT,         70, BUG,       100, 15,   0, PHYSICAL ; increased critical ratio like in Legends: Arceus
endc
...
if DEF(FAITHFUL)
	...
	move CUT,             EFFECT_NORMAL_HIT,         55, STEEL,     100, 20,   0, PHYSICAL
endc
...
if DEF(FAITHFUL)
	...
	move STOMP,           EFFECT_STOMP,              60, ROCK,      100, 15,  30, PHYSICAL ; inspired by Ponyta's "Smash Kick" in TCG GB
endc
...
if DEF(FAITHFUL)
	...
	move HORN_ATTACK,     EFFECT_DEFENSE_DOWN_HIT,   40, ROCK,      100, 15, 100, PHYSICAL ; this move was made for Rhyhorn IMO
endc
...
if DEF(FAITHFUL)
	...
	move WRAP,            EFFECT_TRAP,               25, NORMAL,     90, 20,   0, PHYSICAL
endc
if DEF(FAITHFUL)
...
if DEF(FAITHFUL)
	...
	move POISON_STING,    EFFECT_POISON_HIT,         25, POISON,    100, 30,  30, PHYSICAL ; we love Bug-types in this household
endc
...
if DEF(FAITHFUL)
	...
	move SING,            EFFECT_SLEEP,               0, NORMAL,     70, 15,   0, STATUS
	move SUPERSONIC,      EFFECT_CONFUSE,             0, NORMAL,     75, 20,   0, STATUS ; confuse is way worse than sleep, why was it 65
endc
...
if DEF(FAITHFUL)
	...
	move HYDRO_PUMP,      EFFECT_NORMAL_HIT,        110, WATER,      85,  5,   0, SPECIAL ; slight accuracy nudge to equal Fire Blast
endc
...
if DEF(FAITHFUL)
	...
	move BLIZZARD,        EFFECT_FREEZE_HIT,        110, ICE,        75,  5,  10, SPECIAL ; matches Thunder this way, even if I hate Blizzard
endc
...
if DEF (FAITHFUL)
	...
	move HYPER_BEAM,      EFFECT_HYPER_BEAM,        180, NORMAL,    100,  5,   0, SPECIAL ; BIG BANG ATTACK
endc
...
if DEF(FAITHFUL)
	...
	move STRENGTH,        EFFECT_ATTACK_UP_HIT,      50, FIGHTING,  100, 15, 100, PHYSICAL ; pseudo Power-Up Punch
endc
	move ABSORB,          EFFECT_LEECH_HIT,          20, GRASS,     100, 25,   0, SPECIAL
if DEF(FAITHFUL)
	move MEGA_DRAIN,      EFFECT_LEECH_HIT,          40, GRASS,     100, 15,   0, SPECIAL
else
	move MEGA_DRAIN,      EFFECT_LEECH_HIT,          45, GRASS,     100, 15,   0, SPECIAL ; bit more oomph for what's usually a mid-tier move
endc
...
if DEF(FAITHFUL)
	...
	move RAZOR_LEAF,      EFFECT_NORMAL_HIT,         55, GRASS,     100, 25,   0, PHYSICAL ; 95 accuracy only exists to make people upset
endc
...
if DEF(FAITHFUL)
	...
	move POISONPOWDER,    EFFECT_POISON,              0, POISON,     90, 30,   0, STATUS ; base poisoning is already kind of bad
	move STUN_SPORE,      EFFECT_PARALYZE,            0, GRASS,      90, 25,   0, STATUS ; alternate Thunder Wave with more counterplay
endc
...
if DEF(FAITHFUL)
	...
	move STRING_SHOT,     EFFECT_SPEED_DOWN_HIT,     25, BUG,       100, 30, 100, SPECIAL ; we love Bug-types in this household
endc
...
if DEF(FAITHFUL)
	...
	move THUNDER,         EFFECT_THUNDER,           110, ELECTRIC,   75, 10,  30, SPECIAL ; very slight accuracy nudge outside of rain
	move ROCK_THROW,      EFFECT_NORMAL_HIT,         50, ROCK,      100, 15,   0, PHYSICAL ; it can stand to be 100 accuracy IMO
endc
...
if DEF(FAITHFUL)
	...
	move SCALD,           EFFECT_BURN_HIT,           65, FLYING,    100, 10,  30, SPECIAL ; reworked as "Steam Shot" - change learnsets later
;	move SCALD,           EFFECT_ATTACK_DOWN_HIT,    60, WATER,     100, 15, 100, SPECIAL
endc
...
if DEF(FAITHFUL)
	...
	move VENOSHOCK,       EFFECT_CONDITIONAL_BOOST,  60, POISON,    100, 10,   0, SPECIAL ; Technician Beedrill supremacy
endc
...
if DEF(FAITHFUL)
	...
	move FIRE_BLAST,      EFFECT_BURN_HIT,          110, FIRE,       85,  5,  30, SPECIAL ; Fire deserves a good burning attack - fuck Scald
endc
...
if DEF(FAITHFUL)
	...
	move GLARE,           EFFECT_PARALYZE,            0, DARK,      100, 20,   0, STATUS ; less PP, type change for flavor
endc
...
if DEF(FAITHFUL)
	...
	move DRAINING_KISS,   EFFECT_LEECH_HIT,          50, FAIRY,     100, 15,   0, SPECIAL
endc
...
if DEF(FAITHFUL)
	...
	move DIZZY_PUNCH,     EFFECT_CONFUSE_HIT,        60, FAIRY,     100, 10,  20, PHYSICAL ; more physical Fairy is more good
endc
	move SPORE,           EFFECT_SLEEP,               0, GRASS,     100, 15,   0, STATUS
if DEF(FAITHFUL)
	move FLASH,           EFFECT_ACCURACY_DOWN,       0, NORMAL,    100, 20,   0, STATUS
else
	move FLASH,           EFFECT_ACCURACY_DOWN_HIT,  20, ELECTRIC,  100, 15, 100, SPECIAL ; Electric-type Mud Slap
endc
...
if DEF(FAITHFUL)
	...
	move HYPER_FANG,      EFFECT_FLINCH_HIT,         90, NORMAL,     90, 15,  30, PHYSICAL ; literally only Rattata gets this in base Polished
endc
...
if DEF(FAITHFUL)
	...
	move SLASH,           EFFECT_NORMAL_HIT,         80, NORMAL,    100, 20,   0, PHYSICAL ; mild bump in power for being Normal-type
endc
	move SUBSTITUTE,      EFFECT_SUBSTITUTE,          0, NORMAL,     -1, 10,   0, STATUS
if DEF(FAITHFUL)
	move GIGA_IMPACT,     EFFECT_HYPER_BEAM,        150, NORMAL,     90,  5,   0, PHYSICAL
else
	move GIGA_IMPACT,     EFFECT_HYPER_BEAM,        180, NORMAL,    100,  5,   0, PHYSICAL ; beat his ass, young Midoriya
endc
...
if DEF(FAITHFUL)
	...
	move MUD_SLAP,        EFFECT_ACCURACY_DOWN_HIT,  20, GROUND,    100, 15, 100, SPECIAL ; Falkner's worse but overall this should feel better
...
;else
	move ZAP_CANNON,      EFFECT_PARALYZE_HIT,      130, ELECTRIC,   90,  5, 100, SPECIAL ; accurate paralysis attack with VERY low priority
;endc
...
if DEF(FAITHFUL)
	...
	move OUTRAGE,         EFFECT_RAMPAGE,           120, DRAGON,    100,  5,   0, PHYSICAL ; shrinkens your pp
endc
...
;else
	move DYNAMICPUNCH,    EFFECT_PRIORITY_HIT,       95, FIGHTING,   65,  5, 100, PHYSICAL ; increased priority, loses confuse chance
;endc
...
if DEF(FAITHFUL)
	...
	move HIDDEN_POWER,    EFFECT_HIDDEN_POWER,       65, UNKNOWN_T, 100, 10,   0, SPECIAL ; we are NOT going back to 70 BP Hidden Power
endc
